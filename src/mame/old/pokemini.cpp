...
{//printf("B=%X=%X ",m_pm_reg[0x4F],m_pm_reg[0x4E]);
//	static const int levels[4] = { 0, 1, 1, 2 };
	m_pm_reg[0x4E] -= 1;
	/* Check for underflow of timer */
...
WRITE8_MEMBER(pokemini_state::hwreg_w)
{
	...
	case 0x19:  /* Timers 1 speed
		...
		}//printf("REG19=%X=%X ",timer3_enable,timer3_hi_enable);
		break;
	...
	case 0x1C:  /* Timer 3 pre-scale + enable
		...
		if ( ( data & 0x07 ) != ( m_pm_reg[0x1C] & 0x07 ) )
		{
			...
			m_timers.timer3->adjust(attotime::zero, 0, m_maincpu->cycles_to_attotime(cycles));printf("REG1Ctimer3index=%X ",index);
		}
		...
		if ( ( data & 0x70 ) != ( m_pm_reg[0x1C] & 0x70 ) )
		{
			...
			m_timers.timer3_hi->adjust(attotime::zero, 0, m_maincpu->cycles_to_attotime(cycles));printf("REG1Ctimer3HIindex=%X ",index);
		}

		/* Check if timer3 low should be enabled */
		...
		/* Check if timer3 high should be enabled */
	...
	case 0x48:  /* Timer 3 control 1
		...
		m_pm_reg[0x48] = data;
		update_sound();printf("REG48=%X ",data);
		break;
	case 0x49:  /* Timer 3 control 2
		...
		m_pm_reg[0x49] = data;printf("REG49=%X ",data);
		update_sound();
		break;
